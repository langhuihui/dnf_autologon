...
_ROOM_BEGIN
			...
			bool NodeMgr::has_record_node()
			{
				...
				{
					//trace( "------------------------" );
					//trace( _g[ *pvi.first ].type );
					//trace( _g[ *pvi.first ].pos.x );
					//trace( _g[ *pvi.first ].pos.y );
...
//
			...
			*/
			...
			void DestroyBlock::set_start( const vector<Target>& target_group )
			{
				...
				for( size_t i = 0; i < target_group.size(); i++ )
				{
                    ...
                    if( 33 == target_group[ i ].id
						&& target_group[ i ].map_pos.is_init() )
                    {
                        target_group[ i ].map_pos.corrent_assert();
                        assert( target_group[ i ].map_pos.is_init()
                            && "DestroyBlock::set_start:target_group[ i ].map_pos(33)未初始化" );

                        _33_block_list.push_back(
                            target_group[ i ].map_pos );
					}
					...
					if( 33 != target_group[ i ].id 
						&& 1057 != target_group[ i ].id 
						&& 289 != target_group[ i ].id 
						&& 545 != target_group[ i ].id 
						&& 2081 != target_group[ i ].id
						&& 529 != target_group[ i ].id
						&& 273 != target_group[ i ].id
						)
					{
						trace( "发现新id:" );
						trace( target_group[ i ].id );
					}
#endif
				}
			}
				...
				)
			{
                ...
                assert( is_set() 
                    && "DestroyBlock::near_pos:is_set为假,障碍物未设置完成" );
			...
			string Room::get_id_str()
			{
				assert( !_file_name.emtpy()
					&& "Room::get_id_str:_file_name为空" );
				size_t pos = _file_name.find_last_of( '.' );
				assert( pos != string::npos
					&& "Room::get_id_str:_file_name格式不正确" );

				return _file_name.substr( 0, pos );
			}
