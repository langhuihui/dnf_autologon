...
extern "C"
{
__declspec(dllexport) BOOL    WINAPI ImeInquire(LPIMEINFO, LPTSTR lpszUIClass, LPCTSTR lpszOptions);                  
__declspec(dllexport) BOOL    WINAPI ImeConfigure(HKL, HWND, DWORD, LPVOID);          
__declspec(dllexport) DWORD   WINAPI ImeConversionList(HIMC, LPCTSTR, LPCANDIDATELIST, DWORD dwBufLen, UINT uFlag);   
__declspec(dllexport) BOOL    WINAPI ImeDestroy(UINT);                                
__declspec(dllexport) LRESULT WINAPI ImeEscape(HIMC, UINT, LPVOID);                   
__declspec(dllexport) BOOL    WINAPI ImeProcessKey(HIMC, UINT, LPARAM, CONST LPBYTE); 
__declspec(dllexport) BOOL    WINAPI ImeSelect(HIMC, BOOL);                           
__declspec(dllexport) BOOL    WINAPI ImeSetActiveContext(HIMC, BOOL);                 
__declspec(dllexport) UINT    WINAPI ImeToAsciiEx(UINT uVirtKey, UINT uScaCode, CONST LPBYTE lpbKeyState, LPDWORD lpdwTransBuf, UINT fuState, HIMC);      
__declspec(dllexport) BOOL    WINAPI NotifyIME(HIMC, DWORD, DWORD, DWORD);            
__declspec(dllexport) BOOL    WINAPI ImeRegisterWord(LPCTSTR, DWORD, LPCTSTR);        
__declspec(dllexport) BOOL    WINAPI ImeUnregisterWord(LPCTSTR, DWORD, LPCTSTR);      
__declspec(dllexport) UINT    WINAPI ImeGetRegisterWordStyle(UINT nItem, LPSTYLEBUF); 
__declspec(dllexport) UINT    WINAPI ImeEnumRegisterWord(REGISTERWORDENUMPROC, LPCTSTR, DWORD, LPCTSTR, LPVOID);      
__declspec(dllexport) BOOL    WINAPI ImeSetCompositionString(HIMC, DWORD dwIndex, LPCVOID lpComp, DWORD, LPCVOID lpRead, DWORD);              
#if(WINVER >= 0x040A)                   
__declspec(dllexport) DWORD   WINAPI ImeGetImeMenuItems(HIMC, DWORD, DWORD, LPIMEMENUITEMINFO, LPIMEMENUITEMINFO, DWORD);    
#endif                                  
}
